...
from control import returncontrol_function
...
class ZK(object):
    ...
    def get_server_time(self):
        # Return the current server time from the database
        with pyodbc.connect(conn_str) as db_conn:
            cursor = db_conn.cursor()
            cursor.execute("SELECT GETDATE()")
            server_time = cursor.fetchone()[0]
        return server_time
    def checkMachineConnection(self):
        """
        :return: True if the machine's time was successfully retrieved, otherwise False
        """
        command = const.CMD_GET_TIME
        response_size = 1032
        cmd_response = self.__send_command(command, b'', response_size)
    ...
    def live_capture(self, new_timeout=10):
        ...
        self.end_live_capture = False
        # Synchronize time at the start of live capture
        self.set_time(self.get_server_time())
        last_sync_time = datetime.now()
        while returncontrol_function():
            try:
                current_time = datetime.now()
                # Synchronize time every hour
                if (current_time - last_sync_time) >= timedelta(hours=1):
                    server_time = self.get_server_time()
                    self.set_time(server_time)
                    last_sync_time = current_time
                    if self.verbose: print(f"Time synchronized to {current_time}")
